...
async function searchSeries(seriesName) {
    ...
    const exactMatch = results.find(r => r.name.toLowerCase() === seriesName.toLowerCase());
    if (exactMatch)
        return exactMatch;
 ...
 */
async function fetchSeriesPage(seriesId) {
    await rateLimit();
 ...
 */
export async function browseSeries(genre) {
