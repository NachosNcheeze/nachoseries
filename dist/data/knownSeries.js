 ...
 */
export function getKnownSeriesByGenre(genre) {
    return knownSeries.filter(s => s.genre === genre);
}
/**
 * Get a specific known series by name
 */
export function getKnownSeries(name) {
    return knownSeries.find(s => s.name.toLowerCase() === name.toLowerCase());
}
