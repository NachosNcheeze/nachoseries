...
let flareSolverrAvailable: boolean | null = null;

/**
 * Check if we can use FlareSolverr
 */
async function canUseFlareSolverr(): Promise<boolean> {
 ...
 */
function parseSeriesPage(html: string, seriesName: string): SourceSeries | null {
  ...
  if (books.length === 0) {
    ...
    $('a[href*="/work/"]').each((index, el) => {
      ...
      if (/^(previous|next|more|see all|view)/i.test(title)) return;
      ...
      // Avoid duplicates
      if (books.some(b => b.sourceId === sourceId)) return;
 ...
 * Clean author name
 */
function cleanAuthor(text: string): string | undefined {
  if (!text) return undefined;
