...
import type { SourceSeries, SourceResult, ComparisonResult, Discrepancy } from '../types.js';
...
export function compareSources(
  resultA: SourceResult,
  resultB: SourceResult
  ...
  if (seriesA.author && seriesB.author) {
    const authorSimilarity = stringSimilarity.compareTwoStrings(
      seriesA.author.toLowerCase(),
      seriesB.author.toLowerCase()
    );
...
function countTitleMatches(
  booksA: SourceSeries['books'],
  booksB: SourceSeries['books']
): number {
  let matches = 0;
...
export function mergeSeries(
  seriesA: SourceSeries,
  seriesB: SourceSeries,
  preferSource: 'a' | 'b' = 'a'
): SourceSeries {
 ...
 */
export function needsTalpaVerification(result: ComparisonResult): boolean {
  return (
    result.confidence >= config.confidence.manualReview &&
